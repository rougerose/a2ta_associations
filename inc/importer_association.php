...
function importer_association_adresse($id_association, $row) {
	...
	if ($row['code_postal'] or $row['adresse']) {
		...
		if (intval($id_adresse)) {
			...
			if ($adresse_champs['code_postal'] or $gis_champs['region'] or $gis_champs['ville']) {
				...
				if ($complements_adresse['region']) {
					$region = extraire_multi(sql_getfetsel('nom', 'spip_geo_subdivisions', 'code='.sql_quote($complements_adresse['region'])), 'fr');
				} else {
					$region = 'Ø';
				}

				if ($complements_adresse['departement']) {
					$departement = extraire_multi(sql_getfetsel('nom', 'spip_geo_subdivisions', 'code='.sql_quote($complements_adresse['dedepartement'])), 'fr');
				} else {
					$departement = 'Ø';
				}
...
function importer_adresse_completer($region = '', $code_postal = '') {
	...
	if ($region
		and $region_code = sql_getfetsel('code', 'spip_geo_subdivisions', 'nom LIKE '.sql_quote("%$region%"), '', '', '0,1'))
	{
		$complements_adresse['region'] = $region_code;
	}

	if ($code_postal = substr($code_postal, 0, 2)
		and $departement_code = sql_getfetsel('code', 'spip_geo_subdivisions', 'code='.sql_quote("FR-$code_postal"), '', '', '0,1'))
	{
		$complements_adresse['departement'] = $departement_code;
	}
